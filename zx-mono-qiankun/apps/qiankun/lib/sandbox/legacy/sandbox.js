  ...
  function LegacySandbox(name) {
    ...
    var setTrap = function setTrap(p, value, originalValue) {
      ...
      if (process.env.NODE_ENV === 'development') {
        console.warn(
          '[qiankun] Set window.'
            .concat(p.toString(), ' while sandbox destroyed or inactive in ')
            .concat(name, '!')
        );
  ...
  (0, _createClass2.default)(LegacySandbox, [
    {
      ...
      value: function setWindowProp(prop, value, toDelete) {
        if (value === undefined && toDelete) {
          ...
          (0, _typeof2.default)(prop) !== 'symbol'
        ) {
    ...
    },
    {
      ...
      value: function inactive() {
        ...
        if (process.env.NODE_ENV === 'development') {
          console.info(
            '[qiankun:sandbox] '.concat(this.name, ' modified global properties restore...'),
            [].concat(
              (0, _toConsumableArray2.default)(this.addedPropsMapInSandbox.keys()),
              (0, _toConsumableArray2.default)(this.modifiedPropsOriginalValueMapInSandbox.keys())
            )
          );
