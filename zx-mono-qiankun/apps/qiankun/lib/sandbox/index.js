...
function createSandboxContainer(
  ...
  if (window.Proxy) {
    sandbox = useLooseSandbox
      ? new _sandbox.default(appName, globalContext)
      : new _proxySandbox.default(appName, globalContext);
  } else {
    sandbox = new _snapshotSandbox.default(appName);
  } // some side effect could be be invoked while bootstrapping, such as dynamic stylesheet injection with style-loader, especially during the development phase

  var bootstrappingFreers = (0, _patchers.patchAtBootstrapping)(
    appName,
    elementGetter,
    sandbox,
    scopedCSS,
            ...
            while (1) {
              switch ((_context.prev = _context.next)) {
                case 0:
                  ...
                  /* ------------------------------------------ 1. 启动/恢复 沙箱------------------------------------------ */
                  sandbox.active();
                  sideEffectsRebuildersAtBootstrapping = sideEffectsRebuilders.slice(
                    0,
                    bootstrappingFreers.length
                  );
                  sideEffectsRebuildersAtMounting = sideEffectsRebuilders.slice(
                    bootstrappingFreers.length
                  ...
                  mountingFreers = (0, _patchers.patchAtMounting)(
                    appName,
                    elementGetter,
                    sandbox,
                    scopedCSS,
            ...
            while (1) {
              switch ((_context2.prev = _context2.next)) {
                case 0:
                  ...
                  sideEffectsRebuilders = []
                    .concat(
                      (0, _toConsumableArray2.default)(bootstrappingFreers),
                      (0, _toConsumableArray2.default)(mountingFreers)
                    )
                    .map(function (free) {
                      return free();
                    });
